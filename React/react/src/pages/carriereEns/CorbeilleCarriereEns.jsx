...
const CorbeilleCarriereEns = () => {
    ...
    async function getCarrieresSupprimer() {
        try {
            const response = await axios.get(`${import.meta.env.VITE_URL}/carrieresSupprimer`);
    ...
    const delConge = async (id) => {
        swal({
            ...
            text: "Vous risquez de ne plus pouvoir restaurer cette carrière !",
            icon: "warning",
            buttons: {
            ...
            .then((isConfirm) => {
                if (isConfirm) {
                    axios
                        .delete(`${import.meta.env.VITE_URL}/carriere/` + id)
                        .then(function(response) {
                            swal("Supprimé!", "Votre carriere a bien été supprimée.", "success");
            ...
            .then((isConfirm) => {
                if (isConfirm) {
                    ...
                    axios
                        ...
                        .then(function(response) {
                            swal("Restauré!", "Votre carriere a été marqué comme Restauré.", "success");
                            window.location.reload()
                        })
                        .catch(function(error) {
                            console.error(error);
    ...
    const handleRestaureSelected = () => {
        ...
        axios.put(`${import.meta.env.VITE_URL}/carriere/elementRestaure`, { ids: selectedRecords })
            .then(function (response) {
                swal("Restaurées !", "Les carrieres sélectionnés ont été restauré.", "success");
                window.location.reload();
            })
            .catch(function (error) {
                console.error(error);
    ...
    const handleDeleteSelected = () => {
        ...
        if (check.length==0 || selectedRecords.length==0){
            ...
            axios.put(`${import.meta.env.VITE_URL}/carriere/element`, {
                data: selectedRecords,
            })
                .then(function (response) {
                    swal("Supprimées !", "Les carrieres sélectionné ont été supprimé définitivement.", "success");
                    window.location.reload();
                                                        ...
                                                        <OverlayTrigger
                                                            ...
                                                            overlay={
                                                                <Tooltip id={`tooltip-${placement}`}>
                                                                    <strong>Supprimer définitivement plusieurs carrieres</strong>.
                                                                </Tooltip>
                                                            }
                                                        >
                                                                        ...
                                                                        <tr key={index}>
                                                                            ...
                                                                            <td>
                                                                                {['bottom'].map((placement) => (
                                                                                    ...
                                                                                    >
                                                                                        <Button variant="transparent" className='mb-3 ms-3' onClick={()=>restaurerCarriere(element.id)} ><AiFillCheckCircle className="text-success" style={{ cursor:'pointer' }} /></Button>
                                                                                    </OverlayTrigger>
                                                                                ))}
                                                                            </td>
                                                                            <td>
...
export default CorbeilleCarriereEns;