...
import {Alert, Button, Col, Container, Form, Row, Table, OverlayTrigger, Tooltip} from "react-bootstrap";
import {BsSearch} from "react-icons/bs";
import {AiFillCheckCircle} from "react-icons/ai";
import {FaTrashAlt, FaEdit, FaChevronLeft, FaChevronRight} from "react-icons/fa";
...
const FactSupprimer = () => {
    ...
    async function getFactureSupprimer() {
        try {
            setload(true)
            const response = await axios.get(`${import.meta.env.VITE_URL}/facturesSupprimer`);
    ...
    const delFacture = async (id) => {
        swal({
            ...
            text: "Vous risquez de ne plus pouvoir restaurer cette Facture !",
            icon: "warning",
            buttons: {
            ...
            .then((isConfirm) => {
                if (isConfirm) {
                    axios
                        .delete(`${import.meta.env.VITE_URL}/facture/` + id)
                        .then(function(response) {
                            swal("Supprimé!", "Votre facture a bien été supprimée.", "success");
    ...
    const restaurerFacture = async (id) => {
        swal({
            title: "Etes-vous certains de vouloir supprimer ?",
            text: "Vous risquez de ne plus pouvoir restaurer cette Facture !",
            icon: "warning",
            ...
            .then((isConfirm) => {
                if (isConfirm) {
                    ...
                    axios
                        .put(`${import.meta.env.VITE_URL}/facture/${id}/archiverRestaurer`, { archiver: 0 })
                        .then(function(response) {
                            swal("Restauré!", "Votre facture a été marquée comme Restaurée.", "success");
                            window.location.reload()
                        })
                        .catch(function(error) {
                            console.error(error);
    ...
    const handleRestaureSelected = () => {
        ...
        axios.put(`${import.meta.env.VITE_URL}/facture/elementRestaure`, { ids: selectedRecords })
            .then(function (response) {
                swal("Restaurées !", "Les factures sélectionnées ont été restaurées.", "success");
                window.location.reload();
            })
            .catch(function (error) {
                console.error(error);
    ...
    const handleDeleteSelected = () => {
        ...
        if (check.length==0 || selectedRecords.length==0){
            ...
            axios.put(`${import.meta.env.VITE_URL}/facture/element`, {
                data: selectedRecords,
            })
                .then(function (response) {
                    swal("Supprimées !", "Les factures sélectionnées ont été supprimées définitivement.", "success");
                    window.location.reload();
                        ...
                        <div className="pagetitle">
                            ...
                            <nav>
                                <ol className="breadcrumb">
                                    <li className="breadcrumb-item"><NavLink to="/fact">Factures</NavLink></li>
                                    ...
                                    <div className="row">
                                        ...
                                        <div className="card">
                                            <div className="card-header">
                                                <h3 className="card-title">Liste des factures dans la corbeille</h3>
                                            </div>
                                                                ...
                                                                <Form className='d-flex justify-content-center my-3'>
                                                                        ...
                                                                        <OverlayTrigger
                                                                            ...
                                                                            overlay={
                                                                                <Tooltip id={`tooltip-${placement}`}>
                                                                                    <strong>Supprimer définitivement plusieurs factures</strong>.
                                                                                </Tooltip>
                                                                            }
                                                                        >
                                                                    ...
                                                                    {['bottom'].map((placement) => (
                                                                        <OverlayTrigger
                                                                            ...
                                                                            overlay={
                                                                                <Tooltip id={`tooltip-${placement}`}>
                                                                                    <strong>Restaurer plusieurs factures</strong>.
                                                                                </Tooltip>
                                                                            }
                                                                        >
                                                                                        ...
                                                                                        <td>
                                                                                            {['bottom'].map((placement) => (
                                                                                                <OverlayTrigger
                                                                                                    ...
                                                                                                    overlay={
                                                                                                        <Tooltip id={`tooltip-${placement}`}>
                                                                                                            <strong>Restaurer facture</strong>.
                                                                                                        </Tooltip>
                                                                                                    }
                                                                                                >
                                                                                                    <Button variant="transparent" className='mb-3 ms-3' onClick={()=>restaurerFacture(element.id)} ><AiFillCheckCircle className='text-success me-1'  style={{ cursor:'pointer' }}/></Button>
                                                                                                </OverlayTrigger>
                                                                                            ))}
                                                                                        </td>
                                                                                        <td>
