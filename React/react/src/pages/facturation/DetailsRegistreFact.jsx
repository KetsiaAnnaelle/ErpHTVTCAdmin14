...
import {FaEdit, FaFileSignature, FaTrashAlt} from "react-icons/fa";
...
const CreateFacturePdf = ({ fact, paie, etud, echeance }) => {

    const createdDate = new Date(fact.created_at);
    const formatdate = createdDate.toLocaleDateString();
    const total = fact.map(element => (element.total))
    const payeFact = fact.map(element => (element.paye))
    let paye = 0
    // console.log(parseInt(fact.map(element => (<>{element.total}</>))) - parseInt(paye) );
    ...
    return (
        <Document>
            <Page size="A4" style={styles.page}>
                <View style={styles.container}>
                    ...
                    <View>
                        ...
                        <View>
                            ...
                            <Text style={styles.info}>
                                Filière: <Text style={styles.bold}>{etud.formation_id && etud.formation.nomForm}</Text>
                            </Text>
                            <Text style={styles.info}>
                ...
                <View >
                    <Text style={styles.title}>Facture FAC/2024/000{fact.map(element => (<>{element.id}</>))}</Text>
                    <View style={styles.contentss}>
                        
                        <View>
                            <Text style={styles.info}>
                                <Text style={styles.colors}>Date d'échéance :</Text>
                                <Text>{echeance[0]}</Text>
                            </Text>
                        </View>
                    </View>
                </View>
                ...
                <View style={styles.tableContainer}>
                    <View style={styles.tableRow}>
                        ...
                        <Text style={styles.tableCellHeader}>Prix Unitaire Taxes</Text>
                        <Text style={styles.tableCellHeader}>Montant</Text>
                    </View>
                        <View style={styles.tableRow}>
                            <Text style={styles.tableCell}>{etud.formation_id && etud.formation.nomForm}</Text>
                            <Text style={styles.tableCell}>1,00</Text>
                            <Text style={styles.tableCell}>{fact.map(element => (<>{element.total}</>))}</Text>
                            <Text style={styles.tableCell}>{fact.map(element => (<>{element.total}</>))} FCFA</Text>
                        </View>
                    <View style={styles.contentEnd}>
                        <View style={styles.info}>
                            <Text style={styles.colors}>Total</Text>
                            <Text>    {fact.map(element => (<>{element.total}</>))}FCFA</Text>
                        </View>
                        {paie.map((element, index) => {
                                ...
                                paye = paye + element.MontantPaiement 
                                return(
                                        <>
                                            <View style={styles.contentEnd}>
                                                <View style={styles.info}>
                        ...
                        <View style={styles.info}>
                            <Text style={styles.colors}>Montant dû</Text>
                            <Text>    {paye > total 
                                    ? 0
                                    :
                                     total - payeFact- parseInt(paye)} FCFA </Text>
                        </View>
                    </View>
                </View>
                <Text style={styles.bold}>Merci d'utiliser la communication suivante pour votre paiement : <Text>FAC/2024/000{fact.map(element => (<>{element.id}</>))}</Text></Text>
...
function MyVerticallyCenteredModal(props) {
    ...
    // console.log(totals, paye, restant);
...
const DetailsRegistreFact = () => {
    ...
    async function getFactures() {
        try {
            setload(true)
            const response = await axios.get(`${import.meta.env.VITE_URL}/facture/etudiant/${id}`);
    ...
    let nomForm = etud.formation_id && etud.formation.nomForm
    
    async function getEcheance() {
        try {
            const response = await axios.get(`${import.meta.env.VITE_URL}/get-paiement/etud/${id}`);
    ...
    const onSubmit = async (data) => {
        ...
        .then(function (response) {
            ...
            // GetPaiementFacture()
            // setpaiementFacture(paiementFacture)
        })
        .catch(function(error) {
            console.error(error);
        })
                        ...
                        <div className="pagetitle">
                            ...
                            <nav>
                                <ol className="breadcrumb">
                                    <li className="breadcrumb-item"><NavLink to={`/fact`}>Facture</NavLink></li>
                                    <li className="breadcrumb-item active">Voir Facture</li>
                                </ol>
                            </nav>
                        </div>
                                    ...
                                    <div className="row">
                                        ...
                                        <div className="card">
                                            <div className="card-header">
                                                <h3 className="card-title">Facture etudiant</h3>
                                            </div>
                                                                        ...
                                                                        <Modal show={show} size="lg" onHide={handleClose} aria-labelledby="contained-modal-title-vcenter" centered>
                                                                            ...
                                                                            <Form onSubmit={handleSubmit(onSubmit)}>
                                                                                ...
                                                                                <Form.Select aria-label="Default select example" {...register("formation_id", { required: true })} className='mb-3'controlId="fil" >
                                                                                    <option>Choisir la filière</option>
                                                                                    <option value={etud.formation_id}>{etud.formation_id && etud.formation.nomForm}</option>
                                                                                </Form.Select>
                                                                                {errors.formation_id?.type==='required' && <span className='text-danger'>Le nom de la filière est Obligatoire</span>}
                                                                                ...
                                                                                <Form.Select aria-label="Default select example" {...register("Etudiant_id", { required: true })} className='mb-3' controlId="fil" >
                                                                                    <option>Choisir l'étudiant</option>
                                                                                    <option value={etud.id}>{etud.nomEtud+' '+etud.prenomEtud}</option>
                                                                                </Form.Select>
                                                                                {errors.Etudiant_id?.type==='required' && <span className='text-danger'>Le nom de l'etudiant est Obligatoire</span>}
                                                                        ...
                                                                        {['bottom'].map((placement) => (
                                                                            ...
                                                                            >
                                                                                <Button variant="transparent" className='mb-3 ms-3'>
                                                                                    <PDFDownloadLink document={<CreateFacturePdf fact={fact} paie={paiementFacture} echeance={echeance} etud={etud}/>} fileName='Facture'>
...
export default DetailsRegistreFact;