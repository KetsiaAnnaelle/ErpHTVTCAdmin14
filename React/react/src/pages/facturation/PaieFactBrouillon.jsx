...
const PaieFactBrouillon = () => {
    ...
    async function getPaiementBrouillon() {
        try {
            setload(true)
            const response = await axios.get(`${import.meta.env.VITE_URL}/get-paiement-Facture-brouillon`);
    ...
    const delFacture = async (id) => {
        swal({
            ...
            .then((isConfirm) => {
                if (isConfirm) {
                    axios
                        .delete(`${import.meta.env.VITE_URL}/paiement/` + id)
        ...
        swal({
            ...
            .then((isConfirm) => {
                if (isConfirm) {
                    ...
                    axios
                        .put(`${import.meta.env.VITE_URL}/paiement/${id}/archiverRestaurer`, { archiver: 0 })
                        .then(function(response) {
                            swal("Restauré!", "Votre paiement a été marqué comme Restaurer.", "success");
                            window.location.reload()
                        })
                        .catch(function(error) {
                            console.error(error);
    ...
    const handleRestaureSelected = () => {
        ...
        axios.put(`${import.meta.env.VITE_URL}/paiement/elementRestaure`, { ids: selectedRecords })
            .then(function (response) {
                swal("Restauré !", "Les paiements sélectionnés ont été ajouté comme  brouillon.", "success");
                window.location.reload();
            })
            .catch(function (error) {
                console.error(error);
    ...
    const handleDeleteSelected = () => {
        ...
        if (check.length==0 || selectedRecords.length==0){
            ...
            axios.put(`${import.meta.env.VITE_URL}/paiement/element`, {
                data: selectedRecords,
            })
                .then(function (response) {
                    swal("Supprimées !", "Les paiements sélectionnés ont été supprimé définitivement.", "success");
                    window.location.reload();
                                                                                ...
                                                                                return(
                                                                                    <>
                                                                                        <tr key={index}>
                                                                                            ...
                                                                                            <td key={index}>{element.MontantPaiement}</td>
                                                                                        </tr>
                                                                                    </>
                                                                                )
                                                                            })
...
export default PaieFactBrouillon;