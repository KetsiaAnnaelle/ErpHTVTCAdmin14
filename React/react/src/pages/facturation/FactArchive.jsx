...
import {FaChevronLeft, FaChevronRight, FaTrashAlt, FaEdit} from "react-icons/fa";
...
const FactArchive = () => {
    ...
    async function getFacturesArchiver() {
        try {
            setload(true)
            const response = await axios.get(`${import.meta.env.VITE_URL}/facturesArchiver`);
    ...
    const handleSupprimeSelected = () => {
        ...
        axios.put(`${import.meta.env.VITE_URL}/facture/elementSupprime`, { ids: selectedRecords })
            .then(function (response) {
                swal("Supprimées !", "Les factures sélectionnées ont été supprimées.", "success");
                window.location.reload();
            })
            .catch(function (error) {
                console.error(error);
    ...
    const handleDeleteSelected = () => {
        ...
        if (check.length==0 || selectedRecords.length==0){
            <Alert variant="danger">
                <Alert.Heading>Cocher au moins une facture</Alert.Heading>
            </Alert>
        }else {
            axios.put(`${import.meta.env.VITE_URL}/element`, {
                                    ...
                                    <div className="row">
                                        ...
                                        <div className="card">
                                            <div className="card-header">
                                                <h3 className="card-title">Liste des factues archivées</h3>
                                            </div>
                                                                                    ...
                                                                                    <tr key={index}>
                                                                                        ...
                                                                                        <td>
                                                                                            {['bottom'].map((placement) => (
                                                                                                ...
                                                                                                >
                                                                                                    <Button variant="transparent" className='mb-3 ms-3' onClick={()=>delFacture(element.id)} ><FaTrashAlt className='text-danger me-1'  style={{ cursor:'pointer' }}/></Button>
                                                                                                </OverlayTrigger>
                                                                                            ))}
                                                                                        </td>
