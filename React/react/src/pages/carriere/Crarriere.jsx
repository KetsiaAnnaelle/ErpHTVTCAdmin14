...
function MyVerticallyCenteredModal(props) {
    ...
    const updateCarriere = async (data) => {
        ...
        await axios.post(`${import.meta.env.VITE_URL}/carriere/${props.data.id}`,{
            'NouveauPostePerso': data.nouveau,
            'NouveauSalairePerso': data.salaire,
            'AncienPostePerso': data.ancien,
            'MotifPromotionPerso': data.motif,
        })
                ...
                <Form onSubmit={handleSubmit(updateCarriere)}>
                    <>
                        <Form.Group className="mb-3" controlId="ancien" >
                            ...
                            <Form.Control type='text' {...register("ancien", { required: true, minLength:3 })} defaultValue={props.data.AncienPostePerso} />
                        ...
                        <Form.Group className="mb-3" controlId="nouveau" >
                            ...
                            <Form.Control type='text' {...register("nouveau", { required: true, minLength:3 })} defaultValue={props.data.NouveauPostePerso} />
                        ...
                        <Form.Group className="mb-3" controlId="salaire" >
                            ...
                            <Form.Control type='number' {...register("salaire", { required: true, min:0 })} defaultValue={props.data.NouveausalairePerso} />
                        ...
                        <Form.Group className="mb-3" controlId="motif" >
                            ...
                            <Form.Control as="textarea" rows={3} {...register("motif", { required: true, minLength:3 })} defaultValue={props.data.MotifPromotionPerso}/>
...
// fonction pour lister les abscences
    ...
    async function getCarrieres() {
        try {
            setload(true)
            const response = await axios.get(`${import.meta.env.VITE_URL}/carrieres`);
...
// fonction pour créer un personnel
    ...
    const onSubmit = async (data) => {
        ...
        formData.append('AncienPostePerso', data.ancien);
        formData.append('NouveauPostePerso', data.nouveau);
        formData.append('NouveauSalairePerso', data.salaire);
        formData.append('MotifPromotionPerso', data.motif);

        axios.post(`${import.meta.env.VITE_URL}/carrieres`,formData)
            .then(function (response) {
                console.log(response.data);
    ...
    const handleRestauresSelected = () => {
        ...
        axios.put(`${import.meta.env.VITE_URL}/carriere/elementRestaures`, { ids: selectedRecords })
            .then(function (response) {
        ...
        swal({
            ...
            .then((value) => {
                if (value === "supprimer") {
                    ...
                    axios
                        .put(`${import.meta.env.VITE_URL}/carriere/${id}/archiver`, { archiver: 1 })
                        .then(function(response) {
    ...
    const filtercarriere = () => {
        const filteredRecords = seRecords.filter((record) => {
            ...
            const matchNouveauPoste =
                selectedNouveauPoste === '' || record.NouveauPostePerso === selectedNouveauPoste;

            const matchAncienPoste =
                selectedAncienPoste === '' || record.AncienPostePerso === selectedAncienPoste;
                                    ...
                                    <div className="row">
                                        ...
                                        <div className="card">
                                            <div className="card-header">
                                                <h3 className="card-title">Liste des personnels ayant une promotion</h3>
                                            </div>
                                                                    ...
                                                                    <Row className='d-flex justify-content-center mb-3'>
                                                                        ...
                                                                        <Col className='col-md-2'>
                                                                            ...
                                                                            <Form.Select aria-label="Default select example" className='mb-3' value={selectedPerso} onChange={(e) => setSelectedPerso(e.target.value)}>
                                                                                ...
                                                                                {
                                                                                    carr.length !== 0 && (
                                                                                        [...new Set(carr.map((element) => element.nomPerso +' '+ element.prenomPerso))].map((nom, index) => {
                                                                                            return (
                                                                        ...
                                                                        <Col className='col-md-2'>
                                                                            ...
                                                                            <Form.Select aria-label="Default select example" className='mb-3' value={selectedAncienPoste} onChange={(e) => setSelectedAncienPoste(e.target.value)}>
                                                                                ...
                                                                                {
                                                                                    carr.length !== 0 && (
                                                                                        [...new Set(carr.map((element) => element.AncienPostePerso))].map((nom, index) => {
                                                                                            return (
                                                                                                <option key={index} value={nom}>{nom}</option>
                                                                                            );
                                                                                        })
                                                                        ...
                                                                        <Col className='col-md-2'>
                                                                            ...
                                                                            <Form.Select aria-label="Default select example" className='mb-3' value={selectedNouveauPoste} onChange={(e) => setSelectedNouveauPoste(e.target.value)}>
                                                                                ...
                                                                                {
                                                                                    carr.length !== 0 && (
                                                                                        [...new Set(carr.map((element) => element.NouveauPostePerso))].map((nom, index) => {
                                                                                            return (
                                                                                                <option key={index} value={nom}>{nom}</option>
                                                                                            );
                                                                                        })
                ...
                <Modal.Body>
                    <Form onSubmit={handleSubmit(onSubmit)} encType='multipart/form-data format'>
                        <div className='mb-3'>
                            ...
                            {errors.nom?.type==='required' && <span className='text-danger'>Le nom est Obligatoire</span>}</div>
                        <Form.Group className="mb-3" controlId="ancien" >
                            <Form.Label>Ancien poste</Form.Label>
