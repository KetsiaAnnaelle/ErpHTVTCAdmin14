...
const CreatePaiementEnsPdf = ({ fact, paie }) => {
 ...
 <View>
 <Text style={styles.info}>
 <Text style={styles.bolds}>{fact.nomEns + ' ' + fact.prenomEns}</Text>
 </Text>
 <Text style={styles.info}>
 ...
 <Text>{fact.ProchainPaiementEns}</Text>
 </Text>
 </View>
 </View>
 </View>
 ...
 <View style={styles.tableRow}>
 <Text style={styles.tableCell}>{fact.MotifPaiementEns}</Text>
 <Text style={styles.tableCell}>1,00</Text>
 <Text style={styles.tableCell}>{fact.MontantPaiementEns}</Text>
 <Text style={styles.tableCell}>{fact.MontantPaiementEns} FCFA</Text>
 </View>
 <View style={styles.contentEnd}>
 <View style={styles.info}>
 <Text style={styles.bolds}>Payé le {formatdate}</Text>
 <Text style={styles.bold}> {fact.MontantPaiementEns} FCFA</Text>
 </View>
 ...
 <Text style={styles.bolds}> {element.montantEns}FCFA </Text>
 </View>
 </View>
 </>
 )
 ...
 <Text style={styles.bold}>D'ENSEIGNANT </Text>
 </View>
 <View style={styles.padding}>
...
function MyVerticallyCenteredModal(props) {
 ...
 await axios.put(`${import.meta.env.VITE_URL}/update-newPaiementEns/${props.data.id}`,{

 'montantEns': data.MontantPaiementEns,
 })
 ...
 <Form.Control type="number" {...register("MontantPaiementEns", { required: true, min:0 })} defaultValue={props.data.montantEns}/>
 {errors.MontantPaiementEns?.type==='required' && <span className='text-danger'>Le montant est Obligatoire</span>}
...
const VoirPaiementEns = () => {
 ...
 try {
 const response = await axios.get(`${import.meta.env.VITE_URL}/paiementEns/${id}`);
 ...
 axios.post(`${import.meta.env.VITE_URL}/create-newPaiementEns/${id}`,{
 ...
 try {
 setload(true)
 const response = await axios.get(`${import.meta.env.VITE_URL}/newPaiementEns/${id}`);
 ...
 <h1>Voir Paiement Enseignant</h1>
 <nav>
 <ol className="breadcrumb">
 <li className="breadcrumb-item"><NavLink to="/fact">Paiement</NavLink></li>
 <li className="breadcrumb-item active">Voir Paiement Enseignant</li>
 </ol>
 </nav>
 </div>
 ...
 <Card.Body>
 <Card.Text>
 Noms et prenoms enseignant : <span> {fact.nomEns + ' ' + fact.prenomEns}</span>
 </Card.Text>
 <Card.Text>
 ...
 <Card.Text>
 Prochain paiement : <span> {fact.ProchainPaiementEns}</span>
 </Card.Text>
 </Card.Body>
 </Card>
 </Col>
 ...
 <Form.Group className="mb-3" controlId="" >
 <Form.Label>Montant Du Paiement</Form.Label>
 <Form.Control type="number" {...register("MontantPaiementEns", { required: true, min:0 })} />
 {errors.MontantPaiementEns?.type==='required' && <span className='text-danger'>Le montant est Obligatoire</span>}
 {/* {errors.paye?.type === 'min' && <span>Le montant total ne peut pas être inférieur à 0</span>} */}
 </Form.Group>
 ...
 <PDFDownloadLink document={<CreatePaiementEnsPdf fact={fact} paie={paiementFacture}/>} fileName='Facture_Paiement_enseignant'>
 ...
 <tr>
 <td>{formatdate}</td>
 <td>{fact.RefPaiementEns}</td>
 <td>{fact.MoyenPaiementEns}</td>
 <td>{fact.MotifPaiementEns}</td>
 <td>{fact.MontantPaiementEns}</td>
 <td>0</td>
 <td>{fact.MontantPaiementEns}</td>
 </tr>
 </>
 ...
 <td>Montant: {fact.MontantPaiementEns} FCFA</td>
 </tr>
 {
 ...
 <td>Montant: {element.montantEns} FCFA</td>
 </tr>
...
export default VoirPaiementEns;