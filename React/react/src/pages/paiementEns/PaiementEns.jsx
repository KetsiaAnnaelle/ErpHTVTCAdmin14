...
function MyVerticallyCenteredModal(props) {
 ...
 const updatePaiementPerso = async (data) => {

 await axios.post(`${import.meta.env.VITE_URL}/paiementEns/${props.data.id}`,{
 'RefPaiementEns': data.ref,
 'MoyenPaiementEns': data.moyen,
 'MontantPaiementEns': data.montant,
 'MotifPaiementEns': data.motif,
 'ProchainPaiementEns': data.prochain,
 })
 ...
 <Modal.Body>
 <Form onSubmit={handleSubmit(updatePaiementPerso)}>
 <>
 <Form.Group className="mb-3" controlId="ref" >
 <Form.Label>Reference paiement</Form.Label>
 <Form.Control type='text' {...register("ref", { required: true, minLength:3 })} defaultValue={props.data.RefPaiementEns} />
 {errors.ref?.type==='required' && <span className='text-danger'>Ce Champ est Obligatoire</span>}
 {errors.ref?.type==='minLength' && <span className='text-danger'>Ce Champ doit avoir au moins 3 caractères</span>}
 </Form.Group>
 <Form.Group className="mb-3" controlId="montant" >
 <Form.Label>Montant paiement</Form.Label>
 <Form.Control type='number' {...register("montant", { required: true })} defaultValue={props.data.MontantPaiementEns} />
 {errors.montant?.type==='required' && <span className='text-danger'>Ce Champ est Obligatoire</span>}
 </Form.Group>
 <Form.Group className="mb-3" controlId="moyen" >
 <Form.Label>Moyen paiement</Form.Label>
 <Form.Control type='text' {...register("moyen", { required: true, minLength:3 })} defaultValue={props.data.MoyenPaiementEns} />
 {errors.moyen?.type==='required' && <span className='text-danger'>Ce Champ est Obligatoire</span>}
 {errors.moyen?.type==='minLength' && <span className='text-danger'>Ce Champ doit avoir au moins 3 caractères</span>}
 </Form.Group>
 <Form.Group className="mb-3" controlId="motif" >
 <Form.Label>Motif paiement</Form.Label>
 <Form.Control as="textarea" rows={3} {...register("motif", { required: true, minLength:3 })} defaultValue={props.data.MotifPaiementEns} />
 ...
 <Form.Group className="mb-3" controlId="prochain" >
 <Form.Label>Prochain paiement</Form.Label>
 <Form.Control type='date' {...register("prochain", { required: true })} defaultValue={props.data.ProchainPaiementEns} />
 {errors.prochain?.type==='required' && <span className='text-danger'>Ce Champ est Obligatoire</span>}
 </Form.Group>
...
const PaiementEns = () => {
 ...
 const [records, setRecords] = useState(paie)
...
// fonction pour lister les abscences
 ...
 async function getPaiementPersos() {
 try {
 const response = await axios.get(`${import.meta.env.VITE_URL}/paiementEnss`);
...
// fonction pour créer un personnel
 ...
 formData.append('enseignant_id', data.nom);
 formData.append('RefPaiementEns', data.ref);
 formData.append('MontantPaiementEns', data.montant);
 formData.append('MoyenPaiementEns', data.moyen);
 formData.append('MotifPaiementEns', data.motif);
 formData.append('ProchainPaiementEns', data.prochain);

 axios.post(`${import.meta.env.VITE_URL}/paiementEnss`,formData)
 .then(function (response) {
 console.log(response.data);
 ...
 axios.put(`${import.meta.env.VITE_URL}/paiementEns/elementRestaures`, { ids: selectedRecords })
 .then(function (response) {
 swal("Archivé !", "Les paiements sélectionnés ont été archivé.", "success");
 window.location.reload();
 })
 .catch(function (error) {
 console.error(error);
 ...
 axios.put(`${import.meta.env.VITE_URL}/paiementEns/elementSupprime`, { ids: selectedRecords })
 .then(function (response) {
 swal("Supprimé !", "Les paiements sélectionnés ont été supprimé.", "success");
 window.location.reload();
 })
 .catch(function (error) {
 console.error(error);
...
// fonction pour supprimer un paiement
 ...
 const delPaiementPerso = async (id) => {
 swal({
 title: "Êtes-vous certains de vouloir effectuer cette action ?",
 text: "Vous risquez de ne plus pouvoir restaurer ce paiement !",
 icon: "warning",
 buttons: {
 cancel: "Annuler",
 supprimer: {
 ...
 .then((value) => {
 if (value === "supprimer") {
 // Pour mettre à jour le champ 'supprimer' de l'paiementPerso en le mettant à 1
 axios
 .put(`${import.meta.env.VITE_URL}/paiementEns/${id}/supprimer`, { supprimer: 1 })
 .then(function(response) {
 ...
 // Pour mettre à jour le champ 'archiver' de l'paiementPerso en le mettant à 1
 axios
 .put(`${import.meta.env.VITE_URL}/paiementEns/${id}/archiver`, { archiver: 1 })
 .then(function(response) {
 swal("Archivé!", "Votre paiement a été marqué comme archivé.", "success");
 window.location.reload();
 })
 .catch(function(error) {
 console.error(error);
 ...
 swal("Annulé", "Votre paiement n'a pas été modifié.", "info");
 }
 })
 .catch(function(error) {
 console.error(error);
...
//filtrer les paiementPerso

 const filterpaiementPerso = () => {
 ...
 <h1>Paiement Enseignant</h1>
 <nav>
 <ol className="breadcrumb">
 <li className="breadcrumb-item"><NavLink to="/">Accueil</NavLink></li>
 <li className="breadcrumb-item active">Paiement Enseignant</li>
 </ol>
 </nav>
 </div>
 ...
 <h3 className="card-title">Liste des paiements de enseignant</h3>
 </div>
 ...
 <strong>Supprimer plusieurs paements</strong>.
 </Tooltip>
 }
 >
 ...
 overlay={
 <Tooltip id={`tooltip-${placement}`}>
 <strong>Archiver plusieurs paiements</strong>.
 </Tooltip>
 }
 >
 ...
 {
 paie.length !== 0 && (
 [...new Set(paie.map((element) => element.nomEns +' '+ element.prenomEns))].map((nom, index) => {
 return (
 ...
 total=total+element.MontantPaiementEns
 ...
 overlay={
 <Tooltip id={`tooltip-${placement}`}>
 <strong>Voir paiement</strong>.
 </Tooltip>
 }
 >
 <Button variant="transparent" className='mb-3 ms-3' > <Link to={`/enseignant/voir_paiement/${element.id}`}><FaEye className="text-warning" style={{ cursor:'pointer' }} /></Link></Button>
 </OverlayTrigger>
 ))}
 </td>
 <td>
 ...
 >
 <Button variant="transparent" className='mb-3 ms-3' onClick={()=>delPaiementPerso(element.id)} ><FaTrashAlt className="text-danger" style={{ cursor:'pointer' }} /></Button>
 </OverlayTrigger>
 ))}
 </td>
 <td>{formatdate}</td>
 <td key={index}>{element.nomEns +' '+element.prenomEns}</td>
 <td key={index}>{element.RefPaiementEns}</td>
 <td key={index}>{element.MontantPaiementEns}</td>
 <td key={index}>{element.MoyenPaiementEns}</td>
 <td key={index}>{element.MotifPaiementEns}</td>
 <td key={index}>{element.ProchainPaiementEns}</td>
 </tr>
 ...
 <div className="card-footer">
 <p className='text-danger fs-4 fw-3 mt-3'>Total de paiement : {total} FCFA </p>
 <ReactPaginate
 ...
 <Form.Group className="mb-3" controlId="ref" >
 <Form.Label>Reference paiement</Form.Label>
 <Form.Control type='text' {...register("ref", { required: true, minLength:3 })} />
 {errors.ref?.type==='required' && <span className='text-danger'>Ce Champ est Obligatoire</span>}
 {errors.ref?.type==='minLength' && <span className='text-danger'>Ce Champ doit avoir au moins 3 caractères</span>}
 </Form.Group>
 <Form.Group className="mb-3" controlId="montant" >
 <Form.Label>Montant paiement</Form.Label>
 <Form.Control type='number' {...register("montant", { required: true })} />
 {errors.montant?.type==='required' && <span className='text-danger'>Ce Champ est Obligatoire</span>}
 </Form.Group>
 <Form.Group className="mb-3" controlId="moyen" >
 <Form.Label>Moyen paiement</Form.Label>
 <Form.Control type='text' {...register("moyen", { required: true, minLength:3 })} />
 {errors.moyen?.type==='required' && <span className='text-danger'>Ce Champ est Obligatoire</span>}
 {errors.moyen?.type==='minLength' && <span className='text-danger'>Ce Champ doit avoir au moins 3 caractères</span>}
 </Form.Group>
 <Form.Group className="mb-3" controlId="motif" >
 ...
 <Form.Group className="mb-3" controlId="prochain" >
 <Form.Label>Prochain paiement</Form.Label>
 <Form.Control type='date' {...register("prochain", { required: true })} />
 {errors.prochain?.type==='required' && <span className='text-danger'>Ce Champ est Obligatoire</span>}
 </Form.Group>
