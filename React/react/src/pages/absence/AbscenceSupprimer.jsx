...
import {Button, Col, Container, Form, Row, Table, OverlayTrigger, Tooltip, Breadcrumb} from "react-bootstrap";
...
const AbscenceSupprimer = () => {
 const [records, setRecords] = useState([])
 const [searchQuery, setSearchQuery] = useState('');
 const keys=['typeAbs', 'motifAbs','nbreHeureAbs','dateAbs', 'nomEtud', 'nomForm', 'nomCours'];
 ...
 async function getAbscencesSupprimer() {
 try {
 const response = await axios.get(`${import.meta.env.VITE_URL}/abscencesSupprimer`);
 ...
 text: "Vous risquez de ne plus pouvoir restaurer cette abscence !",
 icon: "warning",
 buttons: {
 ...
 if (isConfirm) {
 axios
 .delete(`${import.meta.env.VITE_URL}/abscence/` + id)
 .then(function(response) {
 swal("Supprimé!", "Votre abscence a bien été supprimée.", "success");
 ...
 const restaurerAbscence = async (id) => {
 swal({
 title: "Etes-vous certains de vouloir restaurer ?",
 text: "Vous risquez de ne plus pouvoir restaurer cette abscence !",
 icon: "warning",
 buttons: {
 ...
 .then((isConfirm) => {
 if (isConfirm) {
 // Pour mettre à jour le champ 'supprimer' du abscence en le mettant à 1
 axios
 .put(`${import.meta.env.VITE_URL}/abscence/${id}/archiverRestaurer`, { archiver: 0 })
 .then(function(response) {
 swal("Restauré!", "Votre abscence a été marquée comme Restaurée.", "success");
 window.location.reload()
 })
 .catch(function(error) {
 console.error(error);
 ...
 axios.put(`${import.meta.env.VITE_URL}/abscence/elementRestaure`, { ids: selectedRecords })
 .then(function (response) {
 swal("Restaurées !", "Les absences sélectionnées ont été restaurées.", "success");
 window.location.reload();
 })
 .catch(function (error) {
 console.error(error);
 ...
 axios.put(`${import.meta.env.VITE_URL}/abscence/element`, {
 data: selectedRecords,
 })
 .then(function (response) {
 swal("Supprimées !", "Les absences sélectionnées ont été supprimées définitivement.", "success");
 window.location.reload();
 ...
 <h3 className="card-title">Liste des absences dans la corbeille</h3>
 </div>
 ...
 <strong>Supprimer définitivement plusieurs abscences</strong>.
 </Tooltip>
 }
 >
 ...
 overlay={
 <Tooltip id={`tooltip-${placement}`}>
 <strong>Restaurer plusieurs abscences</strong>.
 </Tooltip>
 }
 >
 ...
 overlay={
 <Tooltip id={`tooltip-${placement}`}>
 <strong>Restaurer abscence</strong>.
 </Tooltip>
 }
 >
 <Button variant="transparent" className='mb-3 ms-3' onClick={()=>restaurerAbscence(element.id)} ><AiFillCheckCircle className="text-success" style={{ cursor:'pointer' }} /></Button>
 </OverlayTrigger>
 ))}
 </td>
 <td>
 ...
 >
 <Button variant="transparent" className='mb-3 ms-3' onClick={()=>delAbscence(element.id)} ><FaTrashAlt className="text-danger" style={{ cursor:'pointer' }} /></Button>
 </OverlayTrigger>
 ))}
 </td>
...
export default AbscenceSupprimer;