...
const AbscsnceArchive = () => {
 ...
 async function getAbscencesArchiver() {
 try {
 const response = await axios.get(`${import.meta.env.VITE_URL}/abscencesArchiver`);
 ...
 text: "Elle reviendra dans la liste d'abscence !",
 icon: "warning",
 buttons: {
 ...
 .then(function(response) {
 swal("Restauré!", "Votre abscence a été marqué comme Restaurée.", "success");
 window.location.reload()
 })
 .catch(function(error) {
 console.error(error);
 ...
 .then(function (response) {
 swal("Restaurées !", "Les absences sélectionnées ont été restauré.", "success");
 window.location.reload();
 })
 .catch(function (error) {
 console.error(error);
 ...
 <h3 className="card-title">Liste des absences archivées</h3>
 </div>
 ...
 overlay={
 <Tooltip id={`tooltip-${placement}`}>
 <strong>Restaurer Absence</strong>.
 </Tooltip>
 }
 >
 <Button variant="transparent" className='mb-3 ms-3' onClick={()=>restaurerAbscence(element.id)} ><AiFillCheckCircle className='text-success me-1' style={{ cursor:'pointer' }}/></Button>
 </OverlayTrigger>
 ))}
 </td>
 <td>
 ...
 overlay={
 <Tooltip id={`tooltip-${placement}`}>
 <strong>Supprimer Absence</strong>.
 </Tooltip>
 }
 >
...
export default AbscsnceArchive;