...
const CorbeilleConge = () => {
    const [records, setRecords] = useState([])
    const [searchQuery, setSearchQuery] = useState('');
    const keys=['prenomEns','typeConge', 'nomEns'];
    ...
    async function getCongesSupprimer() {
        try {
            const response = await axios.get(`${import.meta.env.VITE_URL}/congeEnssSupprimer`);
    ...
    const delConge = async (id) => {
        swal({
            title: "Etes-vous certains de vouloir supprimer ?",
            text: "Vous risquez de ne plus pouvoir restaurer ce congé !",
            icon: "warning",
            buttons: {
            ...
            .then((isConfirm) => {
                if (isConfirm) {
                    axios
                        .delete(`${import.meta.env.VITE_URL}/congeEns/` + id)
                        .then(function(response) {
                            swal("Supprimé!", "Votre congé a bien été supprimée.", "success");
    ...
    const restaurerConge = async (id) => {
        swal({
            ...
            .then((isConfirm) => {
                if (isConfirm) {
                    // Pour mettre à jour le champ 'supprimer' du congé en le mettant à 1
                    axios
                        .put(`${import.meta.env.VITE_URL}/congeEns/${id}/archiverRestaurer`, { archiver: 0 })
                        .then(function(response) {
                            swal("Restauré!", "Votre congé a été marqué comme Restauré.", "success");
                            window.location.reload()
                        })
                        .catch(function(error) {
                            console.error(error);
    ...
    const handleRestaureSelected = () => {
        ...
        axios.put(`${import.meta.env.VITE_URL}/congeEns/elementRestaure`, { ids: selectedRecords })
            .then(function (response) {
                swal("Restaurées !", "Les congés sélectionnés ont été restauré.", "success");
                window.location.reload();
            })
            .catch(function (error) {
                console.error(error);
    ...
    const handleDeleteSelected = () => {
        ...
        if (check.length==0 || selectedRecords.length==0){
            ...
            axios.put(`${import.meta.env.VITE_URL}/congeEns/element`, {
                data: selectedRecords,
            })
                .then(function (response) {
                    swal("Supprimées !", "Les congés sélectionné ont été supprimé définitivement.", "success");
                    window.location.reload();
                        ...
                        <div className="pagetitle">
                            ...
                            <nav>
                                <ol className="breadcrumb">
                                    <li className="breadcrumb-item"><NavLink to="/conge">Congé</NavLink></li>
                                    ...
                                    <div className="row">
                                        <div className="card">
                                            <div className="card-header">
                                                <h3 className="card-title">Liste des congés dans la corbeille</h3>
                                            </div>
                                                ...
                                                <Form className='d-flex justify-content-center my-3'>
                                                        ...
                                                        <OverlayTrigger
                                                            ...
                                                            overlay={
                                                                <Tooltip id={`tooltip-${placement}`}>
                                                                    <strong>Supprimer définitivement plusieurs congés</strong>.
                                                                </Tooltip>
                                                            }
                                                        >
                                                    ...
                                                    {['bottom'].map((placement) => (
                                                        <OverlayTrigger
                                                            ...
                                                            overlay={
                                                                <Tooltip id={`tooltip-${placement}`}>
                                                                    <strong>Restaurer plusieurs congés</strong>.
                                                                </Tooltip>
                                                            }
                                                        >
                                                                            ...
                                                                            <td>
                                                                                {['bottom'].map((placement) => (
                                                                                    <OverlayTrigger
                                                                                        ...
                                                                                        overlay={
                                                                                            <Tooltip id={`tooltip-${placement}`}>
                                                                                                <strong>Restaurer congé</strong>.
                                                                                            </Tooltip>
                                                                                        }
                                                                                    >
                                                                                        <Button variant="transparent" className='mb-3 ms-3' onClick={()=>restaurerConge(element.id)} ><AiFillCheckCircle className="text-success" style={{ cursor:'pointer' }} /></Button>
                                                                                    </OverlayTrigger>
                                                                                ))}
                                                                            </td>
                                                                            <td>
