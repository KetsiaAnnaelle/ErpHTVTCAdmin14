...
// fonction pour lister les abscences
 ...
 try {
 const response = await axios.get(`${import.meta.env.VITE_URL}/personnelsSupprimer`);
 ...
 axios.put(`${import.meta.env.VITE_URL}/personnel/element`, {
 data: selectedRecords,
 })
 .then(function (response) {
 swal("Supprimées !", "Les personnels sélectionnées ont été supprimé définitivement.", "success");
 ...
 axios.put(`${import.meta.env.VITE_URL}/personnel/elementRestaure`, { ids: selectedRecords })
 .then(function (response) {
 swal("Restauré !", "Les personnels sélectionnés ont été restauré.", "success");
 window.location.reload();
 })
 .catch(function (error) {
 console.error(error);
 ...
 if (isConfirm) {
 axios
 .delete(`${import.meta.env.VITE_URL}/personnel/` + id)
 .then(function(response) {
 ...
 axios
 .put(`${import.meta.env.VITE_URL}/personnel/${id}/archiverRestaurer`, { archiver: 0 })
 .then(function(response) {
...
//filtrer les personnels
 ...
 <li className="breadcrumb-item"><NavLink to="/recrutement_personnel">Recrutements</NavLink></li>
 ...
 <h3 className="card-title">Liste des recrutements dans la corbeille</h3>
 </div>
 ...
 <strong>Supprimer définitivement plusieurs recrutements</strong>.
 </Tooltip>
 }
 >
 ...
 <td>
 <img
 src={`http://localhost:8000/imagesPerso/${element.imagePerso}`}
 ...
 <p className='text-danger fs-4 fw-3 mt-3'>Total Personnels : {total} </p>
 <ReactPaginate
...
export default PersonnelSupprimer;