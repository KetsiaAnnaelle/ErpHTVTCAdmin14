...
import {Button, Col, Container, Form, Row, Table, OverlayTrigger, Tooltip, Nav} from "react-bootstrap";
...
const StageSupprimer = () => {
    ...
    async function getStageSupprimer() {
        try {
            setload(true)
            const response = await axios.get(`${import.meta.env.VITE_URL}/stagesSupprimer`);
    ...
    const delStage = async (id) => {
        swal({
            ...
            .then((isConfirm) => {
                if (isConfirm) {
                    axios
                        .delete(`${import.meta.env.VITE_URL}/stage/` + id)
    ...
    const restaurerStage = async (id) => {
        swal({
            title: "Etes-vous certains de vouloir supprimé ?",
            text: "Vous risquez de ne plus pouvoir restauré cette stage !",
            icon: "warning",
            buttons: {
            ...
            .then((isConfirm) => {
                if (isConfirm) {
                    ...
                    axios
                        ...
                        .then(function(response) {
                            swal("Restauré!", "Votre stage a été marqué comme Restauré.", "success");
                            window.location.reload()
                        })
                        .catch(function(error) {
                            console.error(error);
    ...
    const handleRestaureSelected = () => {
        ...
        axios.put(`${import.meta.env.VITE_URL}/stage/elementRestaure`, { ids: selectedRecords })
            .then(function (response) {
                swal("Restauré !", "Les stages sélectionnés ont été restauré", "success");
                window.location.reload();
            })
            .catch(function (error) {
                console.error(error);
    ...
    const handleDeleteSelected = () => {
        ...
        if (check.length==0 || selectedRecords.length==0){
            ...
            axios.put(`${import.meta.env.VITE_URL}/stage/element`, {
                ...
                .then(function (response) {
                    swal("Supprimé !", "Les stages sélectionnés ont été supprimé définitivement.", "success");
                    window.location.reload();
                                    ...
                                    <div className="row">
                                        ...
                                        <div className="card">
                                            <div className="card-header">
                                                <h3 className="card-title">Liste des stages dans la corbeille</h3>
                                            </div>
                                                                        ...
                                                                        <OverlayTrigger
                                                                            ...
                                                                            overlay={
                                                                                <Tooltip id={`tooltip-${placement}`}>
                                                                                    <strong>Supprimer définitivement plusieurs stages</strong>.
                                                                                </Tooltip>
                                                                            }
                                                                        >
                                                                                        ...
                                                                                        <tr key={index}>
                                                                                            ...
                                                                                            <td>
                                                                                                {['bottom'].map((placement) => (
                                                                                                    ...
                                                                                                    >
                                                                                                        <Button variant="transparent" className='mb-3 ms-3' onClick={()=>restaurerStage(element.id)} ><AiFillCheckCircle className="text-success" style={{ cursor:'pointer' }} /></Button>
                                                                                                    </OverlayTrigger>
                                                                                                ))}
                                                                                            </td>
                                                                                            <td>
