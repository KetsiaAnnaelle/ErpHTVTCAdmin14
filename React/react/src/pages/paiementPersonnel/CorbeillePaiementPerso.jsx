...
const CorbeillePaiementPerso = () => {
 ...
 async function getPaiementsSupprimer() {
 try {
 const response = await axios.get(`${import.meta.env.VITE_URL}/paiementPersosSupprimer`);
 ...
 if (isConfirm) {
 axios
 .delete(`${import.meta.env.VITE_URL}/paiementPerso/` + id)
 .then(function(response) {
 swal("Supprimé!", "Votre paiement a bien été supprimée.", "success");
 ...
 text: "Vous risquez de ne plus pouvoir restaurer cet paiement !",
 icon: "warning",
 buttons: {
 ...
 .then((isConfirm) => {
 if (isConfirm) {
 // Pour mettre à jour le champ 'supprimer' du paiement en le mettant à 1
 axios
 ...
 axios.put(`${import.meta.env.VITE_URL}/paiementPerso/element`, {
 data: selectedRecords,
 })
 .then(function (response) {
 swal("Supprimées !", "Les paiements sélectionné ont été supprimé définitivement.", "success");
 window.location.reload();
 ...
 <li className="breadcrumb-item"><NavLink to="/paiement_personnel">Paiement Personnels</NavLink></li>
 ...
 <strong>Supprimer définitivement plusieurs paiements</strong>.
 </Tooltip>
 }
 >
 ...
 >
 <Button variant="transparent" className='mb-3 ms-3' onClick={()=>restaurerPaiement(element.id)} ><AiFillCheckCircle className="text-success" style={{ cursor:'pointer' }} /></Button>
 </OverlayTrigger>
 ))}
 </td>
 <td>
...
export default CorbeillePaiementPerso;