...
function MyVerticallyCenteredModal(props) {
 ...
 await axios.post(`${import.meta.env.VITE_URL}/paiementPerso/${props.data.id}`,{
 'RefPaiementPerso': data.ref,
 'MoyenPaiementPerso': data.moyen,
 'MontantPaiementPerso': data.montant,
 'MotifPaiementPerso': data.motif,
 'ProchainPaiementPerso': data.prochain,
 })
 ...
 <Form.Control type='text' {...register("ref", { required: true, minLength:3 })} defaultValue={props.data.RefPaiementPerso} />
 ...
 <Form.Control type='number' {...register("montant", { required: true })} defaultValue={props.data.MontantPaiementPerso} />
 ...
 <Form.Control type='text' {...register("moyen", { required: true, minLength:3 })} defaultValue={props.data.MoyenPaiementPerso} />
 ...
 <Form.Control as="textarea" rows={3} {...register("motif", { required: true, minLength:3 })} defaultValue={props.data.MotifPaiementPerso} />
 ...
 <Form.Control type='date' {...register("prochain", { required: true })} defaultValue={props.data.ProchainPaiementPerso} />
...
// fonction pour lister les abscences
 ...
 try {
 const response = await axios.get(`${import.meta.env.VITE_URL}/paiementPersos`);
...
// fonction pour créer un personnel
 ...
 formData.append('RefPaiementPerso', data.ref);
 formData.append('MontantPaiementPerso', data.montant);
 formData.append('MoyenPaiementPerso', data.moyen);
 formData.append('MotifPaiementPerso', data.motif);
 formData.append('ProchainPaiementPerso', data.prochain);

 axios.post(`${import.meta.env.VITE_URL}/paiementPersos`,formData)
 .then(function (response) {
 console.log(response.data);
 ...
 axios.put(`${import.meta.env.VITE_URL}/paiementPerso/elementRestaures`, { ids: selectedRecords })
 .then(function (response) {
...
// fonction pour supprimer un paiement
 ...
 axios
 .put(`${import.meta.env.VITE_URL}/paiementPerso/${id}/archiver`, { archiver: 1 })
 .then(function(response) {
...
//filtrer les paiementPerso
 ...
 <h1>Paiement Personnel</h1>
 <nav>
 <ol className="breadcrumb">
 <li className="breadcrumb-item"><NavLink to="/">Accueil</NavLink></li>
 <li className="breadcrumb-item active">Paiement Personnel</li>
 </ol>
 </nav>
 </div>
 ...
 <h3 className="card-title">Liste des paiements de personnel</h3>
 </div>
 ...
 {
 paie.length !== 0 && (
 [...new Set(paie.map((element) => element.nomPerso +' '+ element.prenomPerso))].map((nom, index) => {
 return (
 ...
 >
 <Button variant="transparent" className='mb-3 ms-3' > <Link to={`/paiement_personnel/voir_paiement_personnel/${element.id}`}><FaEye className="text-warning" style={{ cursor:'pointer' }} /></Link></Button>
 </OverlayTrigger>
 ))}
 </td>
 <td>
...
export default PaiementPersonnel;