...
const ArchiveCongeEns = () => {
    ...
    async function getCongesArchiver() {
        try {
            const response = await axios.get(`${import.meta.env.VITE_URL}/congeEnssArchiver`);
    ...
    const delConge = async (id) => {
        swal({
            ...
            text: "Vous risquez de ne plus pouvoir restauré ce congé!",
            icon: "warning",
            buttons: {
            ...
            .then((isConfirm) => {
                if (isConfirm) {
                    // Pour mettre à jour le champ 'supprimer' de l'congé en le mettant à 1
                    axios
                        .put(`${import.meta.env.VITE_URL}/congeEns/${id}/supprimer`, { supprimer: 1 })
                        .then(function(response) {
                            swal("Supprimé!", "Votre congé a bien été supprimé.", "success");
    ...
    const restaurerConge = async (id) => {
        swal({
            ...
            text: "Elle reviendra dans la liste de congé !",
            icon: "warning",
            buttons: {
    ...
    const handleSupprimeSelected = () => {
        ...
        axios.put(`${import.meta.env.VITE_URL}/congeEns/elementSupprime`, { ids: selectedRecords })
            .then(function (response) {
                swal("Supprimées !", "Les congés sélectionnés ont été supprimé.", "success");
                window.location.reload();
            })
            .catch(function (error) {
                console.error(error);
                                        ...
                                        <section className="content">
                                            <div className="card">
                                                <div className="card-header my-3">
                                                    <h3 className="card-title">Liste des congés archivés</h3>
                                                </div>
                                                                            ...
                                                                            <tr key={index}>
                                                                                ...
                                                                                <td>
                                                                                    {['bottom'].map((placement) => (
                                                                                        ...
                                                                                        >
                                                                                            <Button variant="transparent" className='mb-3 ms-3' onClick={()=>restaurerConge(element.id)} ><AiFillCheckCircle className='text-success me-1'  style={{ cursor:'pointer' }}/></Button>
                                                                                        </OverlayTrigger>
                                                                                    ))}
                                                                                </td>
                                                                                <td>
...
export default ArchiveCongeEns;