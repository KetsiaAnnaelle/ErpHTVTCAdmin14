...
import { BsSearch, BsFillPencilFill, BsArrowDownUp } from 'react-icons/bs';
...
//  const [nbre, setnbre] = useState(1)
 ...
 // Récupérer les 2 derniers chiffres de l'année
//  const year = new Date().getFullYear().toString().slice(-2)

 // Générer la référence en concaténant "REF", les 2 derniers chiffres de l'année et l'ID du paiement
//  const [reference, setreference] = useState(`REF${year}${nbre}`)
 ...
 const keys=['nomEtud', 'nomForm','RefPaiement', 'MontantPaiement', 'MoyenPaiement', 'MotifPaiement', 'ProchainPaiement'];
 ...
 const [allpaiebydate, setallpaiebydate] = useState([]) //pour avoir les etudiants en fonction de la date d'inscription //pour afficher tous les etudiants issus du filtre de la filiere dans le tableau
    ...
    let reference = uuidv4().replace(/-/g, '').substr(0, 10);
    ...
    const onSubmit = async (data) => {
        ...
        // Mettre à jour la valeur du champ "RefPaiement" dans le formulaire
        data.RefPaiement = reference;

        formData.append('RefPaiement', reference);
        ...
        axios.post(`${import.meta.env.VITE_URL}/create-paiement`,formData)
        .then(function (response) {
            console.log(response.data);
    ...
    function setFiltreAllMoyensPaiement(mot) {

        const filteredStudents = newarr.filter(et => et.MoyenPaiement == mot)
        setallPaiement(filteredStudents);
        // console.log(et);
        setShowFilter(false)
    }

    function setFiltreAllMotifs(mot) {

    const filteredStudents = newarr.filter(et => et.MotifPaiement == mot)
    setallPaiement(filteredStudents);
    setShowFilter(false)
    }
 ...
 //Modifier les infos d'un paiement
    ...
    async function Archiver(id) {
        ...
        swal("Paiement mis en brouillon avec succès !!!",{
        icon:"success",
        });
    ...
    const handleDeleteSelected = () => {
        ...
        swal({
        title: `Voulez-vous vraiment supprimer ce/ces paiement(s)?`,
        icon: "warning",
        buttons: true,
        dangerMode:true,
        })
        ...
        swal("Paiement(s) supprimé(s) definitivement !!!",{
        icon:"success",
        });
        })
        .catch(function (error) {
    ...
    function filtreDateFin(mot) {
        setdateFin(mot)

        let element=newarr.filter((item)=>item.created_at.split(' ')[0]>=dateDebut && item.created_at.split(' ')[0]<=mot)

        setallPaiement(element)
    }

    function filtreDateDebut(mot) {
        setdateDebut(mot)
        setstate(false)
    }
    ...
    <Button variant="transparent" className='mb-3 ms-3' onClick={handleShowFilter} data-bs-toggle="modal" data-bs-target="#staticBackdrop"><FaFilter className="text-primary" size={24} style={{ cursor:'pointer' }}/></Button>
        ...
        {['bottom'].map((placement) => (
            ...
            >
                <Link variant="transparent" to={'/fact/paie_archiver'} className='btn btn-transparent'>Brouillon</Link>
            </OverlayTrigger>
        ))}
        {['bottom'].map((placement) => (
            <OverlayTrigger
            ...
            >
                <Link to={'/fact/paie_corbeille'}  className='btn btn-transparent'>Corbeille</Link>
            </OverlayTrigger>
        ))}
    ...
    <Form.Control type="date" value={dateDebut} onChange={(e)=>filtreDateDebut(e.target.value)}/>
    </Form.Group>
    </Col>

    <Col className='col-md-4'>
    <Form.Group className="mb-3" controlId="dateFin">
    <Form.Control disabled={state} type="date" value={dateFin} onChange={(e)=>filtreDateFin(e.target.value)}/>
    </Form.Group>
    </Col>

    <Col className='col-md-4'>
    <Form.Select aria-label="Default select example" className='mb-3' onChange={(e)=>setFiltreAllFiliere(e.target.value)}>
    <option>Choisir la filière</option>
    {
        leon.length!==0 && (
    ...
    <Col className='col-md-6'>
    <Form.Select aria-label="Default select example" className='mb-3' onChange={(e)=>setFiltreAllMotifs(e.target.value)}>
    <option>Motif</option>
    {
        ...
        newarr.length!==0 && (
            newarr.map((element,index) => {
                return(
                    <option key={index} value={element.MotifPaiement}>{element.MotifPaiement}</option>
                )
            })
        )
    }
    ...
    <Col className='col-md-6'>
        <Form.Select aria-label="Default select example" className='mb-3' onChange={(e)=>setFiltreAllMoyensPaiement(e.target.value)}>
            <option>Moyen de Paiement</option>
            {
                ...
                newarr.length!==0 && (
                    newarr.map((element,index) => {
                        return(
                            <option key={index} value={element.MoyenPaiement}>{element.MoyenPaiement}</option>
                        )
                    })
                )
            }
    ...
    <tr>
        ...
        <th></th>
        <th></th>
        <th>Date Du Paiement <BsArrowDownUp className='fw-bold ms-1'onClick={TrieDateDescendant} style={{ cursor:'pointer' }}/></th>
        <th>Reference</th>
        <th>Filière</th>
        <th>Etudiants</th>
        <th>Montant</th>
    ...
    const EcheanceDate = new Date(element.ProchainPaiement);
    const EcheanceDateFormat = EcheanceDate.toISOString();

    // function joursRestantsAvantDate(Cibledate) {
    //     const dateCible = new Date(Cibledate)
    //     // Convertir la date cible en objet Date
        
    //     // Obtenir la date d'aujourd'hui
    //     const aujourdHui = new Date(element.created_at);
      
    //     // Calculer la différence en millisecondes entre aujourd'hui et la date cible
    //     let differenceEnMs = dateCible - aujourdHui;
    //     console.log(differenceEnMs);
        
    //     // Si la date cible est déjà passée, retourner 0 jours restants
    //     if (differenceEnMs < 0) {
    //       // Convertir la différence en millisecondes en jours écoulés et la rendre positive
    //         let joursEcoules = Math.ceil(Math.abs(differenceEnMs) / (1000 * 60 * 60 * 24));
    //         // On veut afficher le nombre de jours écoulés en partant de la date cible
    //         joursEcoules = joursEcoules - 1; // Décrémenter d'un jour
    //         return `La date est passée depuis ${joursEcoules} jours.`;
    //     }
    ...
    //         // Tant que la différence en millisecondes est positive (c'est-à-dire que la date cible n'a pas encore été atteinte)
    //         while (differenceEnMs > 0) {
    //           // Incrémenter le nombre de jours restants
    //           joursRestants = joursRestants+1;
              
    //           // Ajouter un jour en millisecondes
    //           aujourdHui.setDate(aujourdHui.getDate() + 1);
          
    //           // Recalculer la différence en millisecondes
    //           differenceEnMs = dateCible - aujourdHui;
    //         }
    //         return `Il reste ${joursRestants} jours`
    //     }
    ...
    <td>{element.MontantPaiement} FCFA</td>
    <td>{element.MoyenPaiement}</td>
    <td>{element.MotifPaiement}</td>
    <td className="text-success fw-bold">{joursRestantsAvantDate(formatdate,EcheanceDateFormat)}</td>
    <td><FaEdit className='text-primary me-1' onClick={()=>handleMan(element)} style={{ cursor:'pointer' }}/></td>

    <td>
        <FaFileArchive className="text-warning" style={{ color:'red', cursor:'pointer' }} onClick={()=>Archiver(element.id)}/>
    </td>
    <td><FaTrashAlt className="text-danger" style={{ cursor:'pointer' }} onClick={()=>MettreDansLaCorbeille(element.id)}/></td>
    ...
    <Modal show={show1} size="lg" onHide={handleClose1} aria-labelledby="contained-modal-title-vcenter" centered>
            ...
            <Modal.Body>
                <Form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <Form.Group className="mb-3" controlId="" >
                            <Form.Label>Reference</Form.Label>
                            <Form.Control type="text" {...register("RefPaiement", { required: true, min:0 })} defaultValue={reference} readOnly />
                            {errors.RefPaiement?.type==='required' && <span className='text-danger'>La Referemce est Obligatoire</span>}
                        </Form.Group>

                        <Form.Select aria-label="Default select example" {...register("formation_id", { required: true })} className='mb-3' controlId="fil" onChange={(e) => handleFiliereChange(e)}>
                            <option>Choisir la filière</option>
                            {
                                leon.length!=0 && (
                                    leon.Fileres.map((element, index) => {
                        ...
                        <Form.Select aria-label="Default select example" {...register("MotifPaiement", { required: true })} className='mb-3'controlId="" >
