...
class PaiementController extends Controller
{
    ...
    public function showProchainEtud(string $id)
    {
        try {
    ...
    public function paiementTotal()
    {
        try {
            // Récupérer toutes les formations
            $formations = Formation::all();

            // Tableau pour stocker le total des paiements par filière
            $totalPaiementsParFiliere = [];

            foreach ($formations as $formation) {
                // Calculer le total des paiements par le biais des factures
                $totalPaiementsFactures = Facture::whereHas('etudiant', function ($query) use ($formation) {
                ...
                // Calculer le total des paiements par le biais des paiements
                $totalPaiementsPaiements = Paiement::whereHas('etudiant', function ($query) use ($formation) {
                        $query->where('formation_id', $formation->id)
                            ->whereNull('archived_at')
                            ->whereNull('deleted_at');
                    })
                ...
                // Total des paiements par filière (somme des montants de factures et de paiements)
                $totalPaiementsFiliere = $totalPaiementsFactures + $totalPaiementsPaiements;

                // Stocker le total des paiements par filière dans le tableau
                $totalPaiementsParFiliere[$formation->nomForm] = $totalPaiementsFiliere;
            }

            // Convertir l'objet associatif en tableau
            $totalPaiementsParFiliereArray = [];
            foreach ($totalPaiementsParFiliere as $key => $value) {
                $totalPaiementsParFiliereArray[] = ['filiere' => $key, 'total_paiements' => $value];
            }

            return response()->json([$totalPaiementsParFiliereArray]);
